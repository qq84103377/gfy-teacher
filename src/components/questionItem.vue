  ...
  <section class="question-item-wrap">
    ...
    <div class="question-item-wrap__ctn">
      ...
      <div class="icon-group">
        <span v-if="isQuestion"><i class="iconGFY icon-accuracy"></i>{{item.correctRate || item.correntRate || item.errorPersent}}</span>
    ...
    <div class="question-item-wrap__btn-group van-hairline--top" :id="'question-item'+index">
      <van-icon v-if="!isSend" @click="changeToolTip(1)" name="ellipsis" class="fs18"></van-icon>
      <div class="aic" style="flex: 1;justify-content: flex-end">
        <div v-if="isSend" class="btn-item" @click="$emit('correct')">纠错</div>
        <div class="btn-item" :class="{active:analyseShow}" @click="analyseShow=!analyseShow">查看解析
        </div>
        ...
        <div v-if="!isSend" class="btn-item" :class="{active:item.isRemove}" @click="handleAdd">
    ...
    <van-popup v-model="showTool" :get-container="'#question-item'+index" class="tooltip-pop">
      <div class="tooltip-pop-wrap">
        <div v-if="!isSend" class="tooltip-pop-wrap__item" @click="changeToolTip;$emit('correct')">纠错</div>
        <div v-if="up" @click="changeToolTip;$emit('move',0)" class="tooltip-pop-wrap__item">上移</div>
        <div v-if="down" @click="changeToolTip;$emit('move',1)" class="tooltip-pop-wrap__item">下移</div>
      </div>
    </van-popup>
    <van-dialog v-model="showDelDialog" message='确定移除当前试题？' show-cancel-button @cancel='cancelDel' @confirm='confirmDel'>
      <!-- <p class="del-text">确定移除当前试题？</p> -->
    </van-dialog>
  </section>
</template>
...
export default {
  name: "questionItem",
  props: ['isSend', 'index', 'isQuestion', 'up', 'down', 'item', 'showDel','showTooltip'],//isQuestion 是否试题页面适用
  data() {
    return {
      analyseShow: false,
      // tooltip: false,
  ...
  mounted() {
    ...
    // setInterval(() => {
    //   console.log(this.item,"interval")
    // }, 10000);
  },
  computed: {
    showDelDialog: {
