<template>
  <section class="exam-bar">
    <div class="exam-choice" @click="changePop"><i class="iconGFY icon-file"></i><span class="badge">{{total}}</span></div>
    ...
    <van-overlay class-name="exam-bar-overlay" :show="selectPop" @click="changePop" />
    ...
    <van-popup v-model="showAddExam" position="bottom">
      <div class="add-exam-wrap">
        <div class="add-exam-wrap__title">
          <div>生成试卷</div>
          <van-icon name="close" @click="changeAddExam"></van-icon>
        </div>
        ...
        <van-cell class="add-exam-wrap__cell">
          <div slot="title" class="aic">
            ...
            <van-radio-group style="display: flex;" v-model="form.difficult">
              <van-radio name="D01" class="mgr10"><i slot="icon" slot-scope="props" :class="['iconGFY','icon-radio-active',{'radio-normal':!props.checked}]"></i>
                易
              </van-radio>
              <van-radio name="D02" class="mgr10"><i slot="icon" slot-scope="props" :class="['iconGFY','icon-radio-active',{'radio-normal':!props.checked}]"></i>
                中
              </van-radio>
              <van-radio name="D03" class="mgr10"><i slot="icon" slot-scope="props" :class="['iconGFY','icon-radio-active',{'radio-normal':!props.checked}]"></i>
                难
              </van-radio>
            </van-radio-group>
          </div>
        ...
        <van-cell class="add-exam-wrap__cell">
          <div slot="title" class="aic">
            ...
            <van-radio-group style="display: flex;" v-model="form.share">
              <van-radio name="S01" class="mgr10"><i slot="icon" slot-scope="props" :class="['iconGFY','icon-radio-active',{'radio-normal':!props.checked}]"></i>
                仅自己
              </van-radio>
              <van-radio name="S02" class="mgr10"><i slot="icon" slot-scope="props" :class="['iconGFY','icon-radio-active',{'radio-normal':!props.checked}]"></i>
                校内
              </van-radio>
              <van-radio name="S03" class="mgr10"><i slot="icon" slot-scope="props" :class="['iconGFY','icon-radio-active',{'radio-normal':!props.checked}]"></i>
                公开
              </van-radio>
            </van-radio-group>
          </div>
    ...
    <filter-panel :visible.sync="showFilterShow" title="添加" :list="tempList" @filter="handleFilter"></filter-panel>
  </section>
</template>
...
export default {
  props: ['type', 'selectList', 'canSelect', 'canAddCourse', 'length', 'qesTypeName', 'knowledgePoint', 'selectPop', 'addExam', 'filterShow'], //length是type为task时需要判断试卷内是否有试题,若无则不能发任务. qesTypeName题型专项进来的题型名字
  name: "examBar",
  ...
  computed: {
    ...
    showAddExam: {
      ...
      set() {
        this.$emit('update:addExam', false)
      }
    },
    showFilterShow: {
      get() {
  ...
  methods: {
    changePop() {
      if (this.selectPop) {
        this.$emit('update:selectPop', false)
      } else {
        this.$emit('update:selectPop', true)
      }
    },
    changeAddExam() {
      this.$emit('update:addExam', false)
