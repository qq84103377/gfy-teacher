  ...
  <section class="board-list">
    ...
    <div class="board-list__body" ref="body">
      <van-pull-refresh v-show="!tabIndex" v-model="teacher.refLoading" @refresh="onRefresh">
        ...
        <van-list v-model="teacher.listLoading" :finished="teacher.finished" :finished-text="teacher.list.length>0?'没有更多了':'当前没有白板～'" @load="onLoad" :offset='80'>
      ...
      if (this.tabIndex) {
        ...
        if (this.firstFlag) {
          this.$nextTick(() => {
            ...
            this.$refs['body'].scrollTop = this.stu.scrollTop
          })
        } else {
          this.firstFlag = true
          this.onLoad()
        ...
        this.$nextTick(() => {
          ...
          this.$refs['body'].scrollTop = this.teacher.scrollTop
        })
      }
    },
    goto(item) {
