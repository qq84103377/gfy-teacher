<template>
  <div class="feedbackList" ref="list">
      ...
      <div>
        <div class="list" v-for="item in list " :key="item.feedbackId" @click="gotoDetail(item)">
          ...
          <div class="txt">
            ...
            <span class="status" :style="{color:handleColor(item.feedbackStatus)}">{{handleStatus(item.feedbackStatus)}}</span>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next(vm => {
      if (vm.$refs['list']) {
        vm.$refs['list'].scrollTop = vm.scroll
      }
    })
  },
