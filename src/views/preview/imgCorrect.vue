<template>
  <section class="img-correct-wrap">
    <i v-if="!isPen&&!isRubber&&(currentImgIndex>0)" class="iconGFY icon-circle-arrow"
       @click="toggle(0)"></i>
    ...
    <draw-board ref="drawBoard" @submitCb="submitCb" :isPen="isPen" :isRubber="isRubber" :imgUrl="imgArr[currentImgIndex].src"></draw-board>
...
<style lang="less" scoped>
  ...
  .img-correct-wrap {
    ...
    &__header {
      ...
      > div {
        .icon-good-white, .icon-top-white, .icon-essence-white, .icon-circle-plus-white, .icon-circle-sub-white, .icon-talk-white {
          margin-right: 2px;
        }
      }
