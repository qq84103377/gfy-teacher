<template>
  <div class="login">
    <div class="login-box" v-if='!isChoose' :style="{background: 'url(' + bg + ') repeat center /100% 100%' }">
      <h1>
        <img class="logo" v-if="!schoolLogo" src="@/assets/img/logo.png" alt="高分云教师app">
        <img class="logo" v-else :src="schoolLogo" alt="高分云教师app">
      </h1>
      <div class="form">
        <div class="class-info">
          <h3>{{schoolName}}</h3>
          <p>{{className|getGradeName}}</p>
        </div>
        <div class="form-detail" v-if='isMobileLogin===true'>
          ...
          <span class="service-text">注册表示同意相关《<i @click="$router.push('/serviceSheet')">用户协议</i>》和《<i
            @click="$router.push('/privateSheet')">隐私政策</i>》</span>
        </div>
...
<script>
  ...
  import {getQrcodeInfo} from '@/api/joinClass'
  import {getGradeName} from "@/utils/filter";
    ...
    methods: {
      ...
      login() {
        ...
        let json = {
          ...
          'roleType': this.roleType,//角色类型A02:老师，A03:学生
      ...
      userLogin() {
        ...
        doUserLogin(params).then(res => {
          ...
          if (res.flag) {
            ...
            if (res.data[0].loginInfoVo.usrInfo.roleType != this.roleType) {
              this.$toast(`请输入${this.roleType == 'A02' ? '老师' : '学生'}账号`);
              return;
            }
            this.$store.commit('setUserInfo', res.data[0].loginInfoVo.usrInfo)
      ...
      getQrcodeInfo() {
        ...
        getQrcodeInfo(params).then(res => {
          if (res.data) {
            ...
            this.className = res.data.qrCodeInfo.classGrade;
            this.schoolLogo = res.data.schoolInfo.backgroundLogoUrl;
            this.roleType = res.data.qrCodeInfo.roleType;
            this.schoolId = res.data.schoolInfo.schoolId;
            let tempObj = {
              schoolName: res.data.schoolInfo.schoolName,
              schoolId: res.data.schoolInfo.schoolId,
              classGrade: res.data.qrCodeInfo.classGrade,
              className: res.data.classInfo && res.data.classInfo.className,
              classId: res.data.classInfo && res.data.classInfo.classId,
              qrCodeType: res.data.qrCodeInfo.qrCodeType,
              accountPrefix: res.data.schoolInfo.accountPrefix,
            }
            window.localStorage.setItem('schoolInfo', JSON.stringify(tempObj))
          } else {
            let tempObj = {
              type: 'F01',
              msg: res.msg
