      ...
      <van-pull-refresh v-model="refLoading" @refresh="onRefresh">
        <van-list v-model="listLoading" :finished="finished" finished-text="没有更多了" @load="onLoad" :offset='80'>
          <list-item @clickTo="goto(item)" class="mgt10" style="background: #fff;"
                     @del="modifyTeachCourseRes(item,index)" v-for="(item,index) in list" :key="index"
                     :itemTitle="item.discussName"
                     :can-slide="true">
            <div slot="badge"><i class="iconGFY" :class="{'icon-send': item.stateName}"></i></div>
            <div slot="cover" class="cover"><img src="../../assets/img/icon-chat.png" alt=""></div>
            <div slot="desc">
              <div class="desc-top">
                <i class="iconGFY"
        ...
        <div style="flex: 1;overflow-y: auto">
          <van-cell class="save-pop__cell">
            <div slot="title" class="align aic">
              ...
              <input class="pdlt10" style="flex: 1" v-model="form.name" type="text" maxlength="128"
                     placeholder="请输入主题,字数在128字内">
