<template>
  <section class="add-task">
    ...
    <div class="add-task__body">
      ...
      <van-cell class="add-task__body__cell"
                v-if="['lesson','material'].includes($route.query.type)||($route.query.taskType=='T01'||$route.query.taskType=='T02'||$route.query.taskType=='T04')">
        <div slot="title">
      ...
      <van-cell class="add-task__body__cell"
                v-if="['lesson','material'].includes($route.query.type)||($route.query.taskType=='T02'||($route.query.taskType=='T04'&&$route.query.testPaperId==0))">
        <div slot="title">
          <div class="add-task__body__cell-ctn" :class="{ccc:form.exam,grey9:isEdit}">
            <div :class='{mgr10:isEdit}'>心得:</div>
            <van-checkbox v-if='!isEdit' :disabled="testPaperName?true:false" class="comment-check"
                          v-model="form.comment">
              <i slot="icon" slot-scope="props"
                 :class="['iconGFY','icon-check',{'normal':!props.checked},{'disabled':testPaperName}]"></i>
              学习心得
            </van-checkbox>
        ...
        <div slot="title">
          ...
          <div class="mgt15">
            <div class="select-wrap" v-for="(item,index) in classList" :key="item.classId">
              <div>
                <van-checkbox class="comment-check" style="margin-left: 0;" v-model="item.check" :name="item.classId"
                ...
                <div class="select-wrap-desc" v-show="item.type!='none'">
                  <div v-if="form.object == 1 ">发布范围:
                    <span v-show="item.type=='all'">全部{{isfEducation?'家长':'学生'}}</span>
                    <span v-show="item.type=='part'">部分{{isfEducation?'家长':'学生'}}</span>
    ...
    <div class="add-task__footer">
      <van-button v-if='!isEdit' type="info" class="submit" @click="submitSendTask" :loading="showLoading"
                  loading-text="发布中...">发布
      </van-button>
      <van-button v-else type="info" class="submit" @click="modifyTask" :loading="showLoading" loading-text="提交中...">
        提交
      </van-button>
    </div>
      ...
      async getSubGroupParent(classId) {
        ...
        await getSubGroupParent(params).then(res => {
          ...
          if (res.flag && res.data.length) {
            this.parentList[classId] = [...res.data]
          }
        })
      },
        ...
        if (this.tchCourseInfo) {
          ...
          if (this.isEdit) {
            ...
            if (flag) {
              ...
              this.form.time1 = startDate ? formatTime(new Date(startDate)) : ''
              this.form.time2 = endDate ? formatTime(new Date(endDate)) : ''
            }
