    ...
    <div class="add-task__body">
      <van-cell class="add-task__body__cell">
        <div slot="title" class="add-task__body__cell-ctn mgl5" :class="{ccc:$route.query.isResend}">
          ...
          <input :disabled="$route.query.isResend" v-model="form.name" type="text" maxlength="64" placeholder="请输入名称,字数在64字内">
      ...
      <van-cell class="add-task__body__cell" v-if="['lesson','material','exam'].includes($route.query.type)||($route.query.taskType=='T01'||($route.query.taskType=='T03'&& ($route.query.resourceType==='R02'||$route.query.resourceType==='R03'))||($route.query.taskType=='T04'&&$route.query.testPaperId!=0))">
        <div slot="title">
          ...
          <van-checkbox class="allow-fast mgt10" v-model="form.allowEdit" v-if="form.exam||isReinforce" :name="form.allowEdit">
            <i slot="icon" slot-scope="props" :class="['iconGFY','icon-check',{'normal':!props.checked}]"></i>
            允许{{isfEducation?'家长':'学生'}}交卷后重新修改答案
          </van-checkbox>
                  ...
                  if (this.$route.query.isResend) {
                    ...
                    element.accountNo.forEach(i => {
                      if (key == i) {
                        this.$set(item.classStudent[key], 'active', true)
                        item.classStudent[key].active = true
                        // console.log("active",item.classStudent[key].active)
                      }
                    })
                  }
                }
